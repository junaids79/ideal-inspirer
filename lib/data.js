...
export async function submitEnquiry({ name, email, phone, service, message }) {
  const { error } = await supabase
    // Table columns are (id, name, phone, email, course, created_at) — no
    // "service" column exists, so map the caller's `service` param onto
    // the actual "course" column here.
    .from("enquiries")
    .insert({ name, email, phone, course: service, message });
